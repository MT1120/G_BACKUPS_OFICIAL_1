...
import { deleteTape } from "../api";
import { FaTrash } from "react-icons/fa";
import toast, { Toaster } from "react-hot-toast";

import { getAllTapesByDate } from "../api";
...
export default function TapeView() {
  ...
  const filterTapes = () => {
    ...
    // if (selectedDate) {
    //   console.log("Pass");
    //   getAllTapesByDate(format(selectedDate, "yyyy-MM-dd"))
    //     .then((response) => {
    //       console.log("Pass2:", response.data);
    //       filtered = response.data; // Update filteredTapes with API response
    //     })
                ...
                <tbody className="text-sm text-center">
                  {filteredTapes.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">{item.onCustody}</td>
                      <td className="px-6 py-4">{item.total_backups}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div
                          ...
                          onClick={() =>
                            handleNavigation(`/dTapeView/${item.id}`)
                          }
                        >
                          Ver más
                        </div>
