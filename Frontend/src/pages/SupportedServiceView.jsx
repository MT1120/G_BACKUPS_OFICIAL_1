...
import { getAllSupportedS, getAllSupportedSByDate } from "../api";
import { useNavigate } from "react-router-dom";
// import { deleteSupportedS } from "../api";
// import { FaTrash } from "react-icons/fa";
// import toast, { Toaster } from "react-hot-toast";

import "react-datepicker/dist/react-datepicker.css";

export default function SupportedServiceView() {
  ...
  const [supportedS, setSPS] = useState([]);
  ...
  const filterSPS = () => {
    let filteredSPS = supportedS;
    //console.log(startDate);
    if (searchTerm) {
      filteredSPS = filteredSPS.filter((tape) =>
        tape.element_name.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }

    setFilteredSPS(filteredSPS);
  };

  const filterSPSByDate = () => {
    if (selectedDate && selectedDate2) {
      console.log("Pass");
      getAllSupportedSByDate(
        format(selectedDate, "yyyy-MM-dd"),
        format(selectedDate2, "yyyy-MM-dd")
      )
        .then((response) => {
          console.log("Pass2:", response.data);
          setFilteredSPS(response.data); // Update filteredTapes with API response
                    ...
                    >
                          ...
                          <div>
                            <button
                              ...
                              onClick={async () => {
                                ...
                                if (accepted) {
                                  await deleteSupportedS(item.id);
                      ...
                      <td className="px-6 py-4 text-right">
                        <div
                          href="#"
                          onClick={() =>
                            handleNavigation(`/detailedSupportedS/${item.id}`)
                          }
