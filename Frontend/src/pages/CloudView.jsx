...
import { getAllClouds, getAllCloudsByDate } from "../api";

import { deleteClouds } from "../api";
...
export default function CloudView() {
  ...
  const filterCloud = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter((cloud) =>
        cloud.cloud_name.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    ...
    if (selectedDate && selectedDate2) {
      ...
      getAllCloudsByDate(
        ...
        .then((response) => {
          ...
          setFilteredClouds(response.data); // Update filteredTapes with API response
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <div
                          href="#"
                          onClick={() =>
                            handleNavigation(`/detailedCloud/${item.id}`)
                          }
