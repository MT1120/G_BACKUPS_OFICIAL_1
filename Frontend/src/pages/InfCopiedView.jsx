...
import { getAllInfCopied, getAllInfCopiedByDate } from "../api";

// import { deleteInfCopied } from "../api";
...
export default function InfCopiedView() {
  ...
  const [filteredinfCopied, setFilteredInfCopied] = useState([]);
  ...
  const getInfCopied = () => {
    getAllInfCopied()
      ...
      .then((data) => {
        setInfCopied(data);
        setFilteredInfCopied(data);
        //console.log(data);
      })
      .catch((err) => alert(err));
  };

  const filterInfCopied = () => {
    let filtered = infCopied;
    //console.log(startDate);
    if (searchTerm) {
      filtered = filtered.filter((tape) =>
        tape.element_name.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }

    setFilteredInfCopied(filtered);
  };

  const filterInfCopiedByDate = () => {
    if (selectedDate && selectedDate2) {
      console.log("Pass");
      getAllInfCopiedByDate(
        format(selectedDate, "yyyy-MM-dd"),
        format(selectedDate2, "yyyy-MM-dd")
      )
        .then((response) => {
          console.log("Pass2:", response.data);
          setFilteredInfCopied(response.data); // Update filteredTapes with API response
  ...
  return (
    <div className="w-[100vw] h-screen overflow-x-hidden">
      <Header />
      <NavBarLogin />
      <div className="g-white w-full flex flex-col border-t-2 border-gray-300  relative ">
        <div className="bg-transparent w-full h-[5rem] flex justify-center items-center text-xl font-bold uppercase">
          <h1>Consultar Información copiada</h1>
        </div>
                ...
                <tbody className="text-center">
                  {filteredinfCopied.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">{item.copiedInf}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div
                          href="#"
                          onClick={() =>
                            handleNavigation(`/detailedInfCopied/${item.id}`)
                          }
