...
import { getAllRecoverTape, getAllRecoverTapeByDate } from "../api";
//import { useNavigate } from "react-router-dom";

import { deleteRecoverTape } from "../api";
...
export default function RecoveryHistoryView() {
  ...
  const [recoverTape, setRecoverTape] = useState([]);

  const [searchTerm, setSearchTerm] = useState("");
  const [filteredRH, setFilteredRH] = useState([]);

  const getRecoverTape = () => {
    getAllRecoverTape()
      .then((res) => res.data)
      .then((data) => {
        setRecoverTape(data);
  ...
  const filterTapes = () => {
    ...
    setFilteredRH(filtered);
  };

  const filterRecoverTapeByDate = () => {
    if (selectedDate && selectedDate2) {
      console.log("Pass");
      getAllRecoverTapeByDate(
        format(selectedDate, "yyyy-MM-dd"),
        format(selectedDate2, "yyyy-MM-dd")
      )
        .then((response) => {
          console.log("Pass2:", response.data);
          setFilteredRH(response.data); // Update filteredTapes with API response
    ...
    <div className="w-[100vw] h-screen overflow-x-hidden">
      ...
      <div className="bg-white w-full flex flex-col border-t-2 border-gray-300  relative ">
        <div className="bg-transparent w-full h-[5rem] flex justify-center items-center text-xl font-bold uppercase">
          <h1>Consultar Historial de recuperación</h1>
        </div>
                      ...
                      <td className="px-6 py-4">
                        <button
                          ...
                          onClick={async () => {
                            ...
                            if (accepted) {
                              await deleteRecoverTape(item.id);
