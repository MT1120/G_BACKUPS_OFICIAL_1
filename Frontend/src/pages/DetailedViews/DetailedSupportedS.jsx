...
import { updateSupportedS, getSupportedS } from "../../api";
import api from "../../api";
...
export default function DetailedSupportedS() {
  const { id } = useParams();
...
//   const [serviceType, setServiceType] = useState("");
  ...
  useEffect(() => {
    async function loadSupportedS() {
      if (id) {
        const response = await getSupportedS(id);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (id) {
      await updateSupportedS(id, data);
